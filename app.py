...
def main():
    """Main application."""
    st.markdown(
        '<h1 class="main-header">💰 Personal Finance Survey Analyzer</h1>',
        unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        analysis_option = st.radio("Choose Analysis:",
                                   ["📊 Data Summary",
                                    "💳 Spending",
                                    "💰 Savings",
                                    "📈 Investments",
    ...
    if analysis_option == "📊 Data Summary":
        ...
        if "error" not in analysis:
            ...
            with col1:
                st.metric(
                    "Avg Spending", overview.get(
                        "Average Total Spending", "N/A"))
            with col2:
                st.metric(
                    "Median Spending", overview.get(
                        "Median Total Spending", "N/A"))
            with col3:
            ...
            fig = st.session_state.visualizer.create_spending_charts(
                save_path=None)
            if fig:
                st.pyplot(fig)
        ...
        if "error" not in analysis:
            ...
            with col1:
                st.metric(
                    "Avg Savings", overview.get(
                        "Average Monthly Savings", "N/A"))
            with col2:
                st.metric(
                    "Median Savings", overview.get(
                        "Median Monthly Savings", "N/A"))
            with col3:
            ...
            st.markdown("### 📊 Visualizations")
            fig = st.session_state.visualizer.create_savings_charts(
                save_path=None)
            if fig:
                st.pyplot(fig)
        ...
        if "error" not in analysis:
            if "Cryptocurrency Analysis" in analysis:
                ...
                with col1:
                    st.metric(
                        "Crypto Owners", crypto.get(
                            "Total Crypto Owners", "N/A"))
                with col2:
                    st.metric(
                        "Adoption Rate", crypto.get(
                            "Crypto Adoption Rate", "N/A"))
            ...
            st.markdown("### 📊 Visualizations")
            fig = st.session_state.visualizer.create_investment_charts(
                save_path=None)
            if fig:
                st.pyplot(fig)
        ...
        if "error" not in analysis:
            ...
            st.markdown("### 📊 Visualizations")
            fig = st.session_state.visualizer.create_financial_literacy_charts(
                save_path=None)
            if fig:
                st.pyplot(fig)
        ...
        for idx, (k, v) in enumerate(
                report.get("Executive Summary", {}).items()):
            with cols[idx]:
                st.metric(k, v)
        ...
        fig = st.session_state.visualizer.create_comprehensive_dashboard(
            save_path=None)
        if fig:
            st.pyplot(fig)
