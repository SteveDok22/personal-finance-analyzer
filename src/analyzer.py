...
class FinanceAnalyzer:
    ...
    def get_spending_analysis(self):
        ...
        for col in spending_cols:
            ...
            analysis["Category Breakdown"][category_name] = {
                ...
                "Percentage of Total": format_percentage(self.data[col].sum() / self.data['total_spending'].sum())
            }
        
        # Generate insights
        if spending_cols:
            # Find highest spending category
            category_totals = {col.replace('monthly_spending_', '').title(): self.data[col].sum() for col in spending_cols}
            ...
            if 'annual_income' in self.data.columns:
                ...
                analysis["Insights"].append(f"Average spending-to-income ratio: {format_percentage(spending_ratio)}")
    ...
    def get_savings_analysis(self):
        ...
        if 'annual_income' in self.data.columns:
            ...
            analysis["Insights"].append(f"{format_percentage(high_savers_pct)} of respondents save more than 20% of their income")
    ...
    def get_fintech_adoption_analysis(self):
        ...
        if 'uses_mobile_banking' in self.data.columns and 'owns_crypto' in self.data.columns:
            tech_enthusiasts = self.data[(self.data['uses_mobile_banking'] == True) & (self.data['owns_crypto'] == True)]
    ...
    def get_financial_literacy_analysis(self):
        ...
        medium_literacy = len(self.data[(self.data['financial_literacy_score'] >= 6) & (self.data['financial_literacy_score'] < 8)])
